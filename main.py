...
class CurrencyConverter:
    ...
    def convert_currency(self):
        try:
            form_currency = self.from_var.get()
            to_currency = self.to_var.get()
            amount = float(self.amount_entry.get())

            c_rates = CurrencyRates()

            rate = c_rates.get_rate(form_currency, to_currency)
            converted_amount = amount * rate

            self.result_label.config(text=f'{amount} {form_currency}={converted_amount:.2f}{to_currency}')
        except ValueError:
